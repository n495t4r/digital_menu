...
export default function GuestView({ establishment }) {
  ...
  // Improved color contrast function with default color
  const getContrastColor = (hexColor) => {
    if (!hexColor) return "#000000"; // Fallback to black if no color is provided
  ...
  const bgColor = establishment.color || "#FFFFFF" // Fallback to white if no color is set

  // Handle case when there are no categories or products
    ...
    <>
      ...
      <div className="min-h-screen" style={{ backgroundColor: "white", color: textColor }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-1">
          ...
          <div className="bg-white bg-opacity-90 rounded-lg shadow-lg p-6 mb-6 mb-6 text-sm text-gray-600">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              ...
              <div className="flex items-center">
                ...
                <p>{establishment.address || "Address not provided"}</p>
              </div>
              <div className="flex items-center">
                <Phone className="mr-2" size={15} style={{ color: bgColor }} /> {/* Update 2 */}
                <p>{establishment.phone || "Phone number not available"}</p>
              </div>
          ...
          {/* Ensure categories and products are valid */}
          <main>
            {establishment.categories.map((category) => (
